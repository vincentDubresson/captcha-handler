        ...
        connectedCallback() {
            ...
            piece.addEventListener('pointerdown', () => {
                ...
                window.addEventListener('pointerup', () => {
                    ...
                    const challengeInput = this.querySelector('.captcha_challenge');
                    const challenge = challengeInput ? challengeInput.value : '';
                    const answer = `${position.x}-${position.y}`;
                    const overlay = this.querySelector('.captcha-valid');

                    if (!challenge) return;

                    captchaService.checkAnswer(challenge, answer)
                        .then(data => {
                            if (data.success && overlay) overlay.classList.add('captcha-valid-confirmed');
                        })
